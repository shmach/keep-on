const coaches = {
  max: {
    ...
    images: {
      ...
      angry: '/assets/coaches/max/coach-max-angry.png',
      satisfied: '/assets/coaches/max/coach-max-satisfied.png',
      embarrassed: '/assets/coaches/max/coach-max-embarrassed.png'
    },
    phrases: {
      welcome: [
 ...
 * @param {distractions, minutesIn, minutesLeft, url, distractionRatio} data 
 */
...
function getCoachMoment(context, data) {
  ...
  if (context === 'distraction') {
    ...
    // Only keys 1/2/3/infinity exist — clamp so a missing/zero/NaN count
    // (or a stale/mismatched caller) can never index into `undefined`
    const count = Number.isFinite(Number(distractions)) ? Number(distractions) : 1;
    const distractionCount = count > 3 ? 'infinity' : Math.max(1, count);
  ...
  if (context === 'complete') {
    ...
    // Missing/NaN ratio must not silently fall through to the worst mood
    const ratio = Number.isFinite(distractionRatio) ? distractionRatio : 0;
